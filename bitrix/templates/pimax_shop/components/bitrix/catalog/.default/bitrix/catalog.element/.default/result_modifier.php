...
$productSlider = CIBlockPriceTools::getSliderForItem($arResult, $arParams['ADD_PICT_PROP'], true);
...
//echo '<pre>', print_r($arSKUPropList), '</pre>';
...
if (!empty($arResult['DISPLAY_PROPERTIES']['MANUFACTURER']['VALUE'])) {

	$arResult['DISPLAY_PROPERTIES']['MANUFACTURER']['VALUE'] = CIBlockElement::GetByID($arResult['DISPLAY_PROPERTIES']['MANUFACTURER']['VALUE'])->GetNext();
...
if ($arResult['MORE_PHOTO']) {
	...
	{
		$photo['FULL'] = CFile::ResizeImageGet($photo['ID'], array('width' => 600, 'height' => 600), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true);
		$photo['PREVIEW'] = CFile::ResizeImageGet($photo['ID'], array('width' => 200, 'height' => 200), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true);
		$photo['SHARE'] = CFile::ResizeImageGet($photo['ID'], array('width'=>526, 'height'=>275), BX_RESIZE_IMAGE_EXACT);
	}
}
...
if(CModule::IncludeModule("sale"))
{
	...
	while ($arItems = $dbBasketItems->Fetch())
	{
		$skuElement = CCatalogProduct::GetByID($arItems['PRODUCT_ID']);

		
		$arSelect = Array("ID", "IBLOCK_ID", "NAME", "DATE_ACTIVE_FROM","PROPERTY_*");//IBLOCK_ID и ID обязательно должны быть указаны, см. описание arSelectFields выше
		$arFilter = Array("ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y", "ID" => $skuElement['ID']);
		$res = CIBlockElement::GetList(Array(), $arFilter, false, Array("nPageSize"=>50), $arSelect);
		if ($ob = $res->GetNextElement()){ 
			 
			 $arProps = $ob->GetProperties();
			 if ($arProps['CML2_LINK']['VALUE'] == $arResult['ID']) 
			 {
	...
	//echo '<pre>', print_r($arResult['ID']), '</pre>';
