...
<?else:?>
		...
		<?foreach($arResult["ORDER_BY_STATUS"] as $key => $group):?>
			...
			<?foreach($group as $k => $order):?>
				...
				<div class="bx_my_order">
					...
					<table class="bx_my_order_table shop_table">
						<thead>
							<tr>
								<th><?=GetMessage('SPOL_ORDER')?> <?=GetMessage('SPOL_NUM_SIGN')?><?=$order["ORDER"]["ACCOUNT_NUMBER"]?> <?=GetMessage('SPOL_FROM')?> <?=$order["ORDER"]["DATE_INSERT_FORMATED"];?></th>
								<th style="min-width: 500px; text-align: right;">
							...
							<tr>
								...
								<td>
									...
									<?if($order["ORDER"]["CANCELED"] != "Y"):?>
										<a href="<?=$order["ORDER"]["URL_TO_CANCEL"]?>" style="min-width:140px"class="btn"><?=GetMessage('SPOL_CANCEL_ORDER')?></a>
									<?endif?>

									<a href="<?=$order["ORDER"]["URL_TO_COPY"]?>" style="min-width:140px"class="btn btn-primary"><?=GetMessage('SPOL_REPEAT_ORDER')?></a>
								</td>
							</tr>
						</tbody>
					</table>
	...
	<?else:?>
		<?=GetMessage('SPOL_NO_ORDERS')?><br /><br />
	<?endif?>
