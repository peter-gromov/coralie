...
<div id="<?php echo $strMainID?>" class="tover_view_page element_fade_in">
	...
	<div class="tover_view_header clearfix">
		<p><?=GetMessage("PIMAX_FASHIONSTORE_BYSTRYY_PROSMOTR")?></p>
		<a id="tover_view_page_close" href="javascript:void(0);"><?=GetMessage("PIMAX_FASHIONSTORE_ZAKRYTQ")?><i>X</i></a>
	</div>
	...
	<div class="clearfix">
		<div class="tovar_view_fotos clearfix">
			...
			<?php if (!isset($arResult['OFFERS']) || empty($arResult['OFFERS'])):?>
				<div id="slider1" class="flexslider slider1">
					<ul class="slides">

						<?php foreach ($arResult['MORE_PHOTO'] as $k => $arOnePhoto):?>
							<?php $index = rand();?>
							<li>
								<img <?php if ($k == 0):?>id="<? echo $arItemIDs['PICT']; ?>"<?php endif;?> src="<? echo $arOnePhoto['FULL']['src']; ?>" alt="" />

							</li>
						<?php endforeach;?>
					</ul>
				</div>
				<div id="<? echo $arItemIDs['SLIDER_CONT_ID']; ?>" class="flexslider carousel1">
					<ul class="slides" id="<? echo $arItemIDs['SLIDER_LIST']; ?>">
				...
				<script type="text/javascript">
					...
					$('#<? echo $arItemIDs['SLIDER_CONT_ID']; ?>').flexslider({
						animation: "slide",
						controlNav: false,
						directionNav: true,
						prevText: "",
						...
						slideshow: false,
						direction: "vertical",
						asNavFor: '#slider<? echo $arItemIDs['SLIDER_CONT_ID']; ?>'
					});
					$('#slider<? echo $arItemIDs['SLIDER_CONT_ID']; ?>').flexslider({
						animation: "fade",
						controlNav: false,
						directionNav: true,
						prevText: "",
						...
						animationLoop: false,
						slideshow: false,
						sync: "#<? echo $arItemIDs['SLIDER_CONT_ID']; ?>"
					});
				</script>
					...
					<div id="slider<? echo $arItemIDs['SLIDER_CONT_OF_ID'].$arOneOffer['ID']; ?>" class="flexslider slider1" style="display: <? echo $strVisible; ?>;">
						<ul class="slides">
							...
							<?php foreach ($arOneOffer['MORE_PHOTO'] as $k => $arOnePhoto):?>
								<?php $index = rand();?>
								<li>
								<div style=" position: absolute;right: 0;width: 71px;">
													<?if ($arResult["PROPERTIES"]["SPECIALOFFER"]["VALUE"] === "Y"):?>
            <img 
			style="width:70px;height:30px;margin-bottom:5px;"
			src="/bitrix/images/one1.png" />
        <?endif;?>
								<?if ($arResult["PROPERTIES"]["NEWPRODUCT"]["VALUE"] === "Y"):?>
            <img 
			style="width:70px;height:30px;margin-bottom:5px;"
			src="/bitrix/images/one2.png" />
        <?endif;?>
							<?if ($arResult["PROPERTIES"]["SALELEADER"]["VALUE"] === "Y"):?>
            <img 
					style="width:70px;height:30px;margin-bottom:5px;"
			src="/bitrix/images/one3.png" />
        <?endif;?>
</div>
		<img <?php if ($k == 0):?>id="<? echo $arItemIDs['PICT']; ?>"<?php endif;?> src="<? echo $arOnePhoto['FULL']['src']; ?>" alt="" />
...
?>
		...
		<div class="tovar_view_btn">
			...
			<a class="add_bag <? echo $buyBtnClass; ?>" href="javascript:void(0);"  id="<? echo $arItemIDs['ADD_BASKET_LINK']; ?>" ><i class="fa fa-shopping-cart"></i><?=GetMessage("PIMAX_FASHIONSTORE_V_KORZINU")?></a>
		</div>
		<div class="tovar_view_btn">
