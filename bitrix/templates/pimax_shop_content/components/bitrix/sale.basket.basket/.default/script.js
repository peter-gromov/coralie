...
function updateBasketTable(basketItemId, res)
{
	...
	if (basketItemId !== null)
	{
		...
		newRow.setAttribute('class', 'cart_item');
		...
		for (i = 0; i < arColumns.length; i++)
		{
			...
			else if (arColumns[i] == 'PRICE')
			{
				...
				oCellPrice.setAttribute('class', 'product-price');
			...
			else
			{
				...
				if (arColumns[i] == 'SUM') {
					oCellCustom.setAttribute('class', 'product-subtotal');
		...
		if (bShowDeleteColumn || bShowDelayColumn)
		{
			var oCellControl = newRow.insertCell(-1);
				oCellControl.setAttribute('class', 'product-remove');

			if (bShowDeleteColumn)
				oCellControl.innerHTML = '<a title="Удалить" class="tover_view_page_close" href="' + basketJSParams['DELETE_URL'].replace('#ID#', arItem['ID']) +'"><span class="fa-product-close" aria-hidden="true"></span></a>';
