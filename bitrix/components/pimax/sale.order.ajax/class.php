...
class SaleOrderAjax extends \CBitrixComponent
{
		...
		foreach ($propertyCollection as $property)
		{
			...
			if ($arProperty["TYPE"] == 'LOCATION')
			{
				...
				/*
					...
					Added by pimax.
					Set default city by pimax.location component
					
				*/
				
				if (!empty($_COOKIE['cookie_city']))
				{
					...
					{
				    	$curVal = CSaleLocation::getLocationCODEbyID($vars['ID']);
					}
				}
			}
	...
	/**
	 * Create order object with user and request data.
	...
	protected function createOrder($userId)
	{
		...
		{
			$this->recalculatePayment($order);
			unset($this->arUserResult['CALCULATE_PAYMENT']);
		}
