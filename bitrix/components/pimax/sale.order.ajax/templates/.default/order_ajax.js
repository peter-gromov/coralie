...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		editActivePropsBlock: function(activeNodeMode)
		{
			...
			var address = (new String($('.bx-ui-sls-fake').attr('title'))).split(", ");
			
			$('#soa-property-21').val(address[2]);
			$('#soa-property-20').val(address[0]);
		...
		getPropertyRowNode: function(property, propsItemsContainer, disabled)
		{
			...
			else
			{
				...
				propsItemNode.setAttribute('id', 'data-property-id-row-' + property.getId());
				propsItemNode.appendChild(label);
			}
			
			// Дата доставки
			if (property.getId() == 11) {
				
				this.insertDateDeliveryProperty(property, propsItemNode, disabled);
				
			// Дата самовывоза
			} else if (property.getId() == 10) {
				
				this.insertDatePickupProperty(property, propsItemNode, disabled);
		...
		insertDateDeliveryProperty: function(property, propsItemNode, disabled)
		{
			var prop, dateInputs, values, i,
				propContainer, inputText;
			...
			else
			{
				...
				for (i = 0; i < inputText.length; i++) 
				{
					...
					this.alterDateDeliveryProperty(property.getSettings(), inputText[i]);
				}
		...
		alterDateDeliveryProperty: function(settings, inputText)
		{
			var parentNode = BX.findParent(inputText, {tagName: 'DIV'});
			var propContainer = BX.create('DIV', {props: {className: 'date-delivery-select'}});
			
			var property, day, month, number, i;
			
			var days = ['Вс', 'Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб'];
			var months = ['Янв', 'Фев', 'Мар', 'Апр', 'Мая', 'Июн', 'Июл', 'Авг', 'Сен', 'Окт', 'Ноя', 'Дек'];
			var allowedDays = [1, 3, 5, 6];
			...
			while(printedDays < 5) 
			{
				if (!printedDays && currentDate.getHours() < 20) 
				{
					today = true;
				} 
				
				currentDate.setDate(currentDate.getDate() + 1);
				if (inArray(currentDate.getDay(), allowedDays)) 
				{
					
					var classNameVal = (!printedDays ? "selected" : "");
				
					if (today) {
						classNameVal += " today";
					}
					
					property = BX.create('DIV', {
						props: {
							className: classNameVal
						},
						attrs: {
							'data-prop-value': (currentDate.getDate() > 9 ? currentDate.getDate() : "0" + currentDate.getDate()) + "." + (currentDate.getMonth()+1 > 9 ? currentDate.getMonth()+1 : "0" + (currentDate.getMonth()+1)) + "." + currentDate.getFullYear()
						}
					});
					
					day = BX.create('SPAN', {text: days[currentDate.getDay()]});
					number = BX.create('p', {text: currentDate.getDate()});
					month = BX.create('SPAN', {text: months[currentDate.getMonth()]});		
					
					property.appendChild(day);
					property.appendChild(number);
					property.appendChild(month);
					propContainer.appendChild(property);
					
					printedDays++;
					
					if (printedDays == 1) {
						firstDay = (currentDate.getDate() > 9 ? currentDate.getDate() : "0" + currentDate.getDate()) + "." + (currentDate.getMonth()+1 > 9 ? currentDate.getMonth()+1 : "0" + (currentDate.getMonth()+1)) + "." + currentDate.getFullYear();
					}
			...
			$(inputText).attr('type', 'hidden').attr('id', 'ORDER_PROP_11').val(firstDay);
			BX.remove(parentNode.querySelector('input[type=button]'));
			
			parentNode.appendChild(propContainer);
			
			
			$(document).on('click', '#data-property-id-row-11 .date-delivery-select div', function () {
				
				$('#ORDER_PROP_11').val($(this).data('prop-value'));
				$('.date-delivery-select div').removeClass('selected');
				$(this).addClass('selected');
				
				if ($(this).hasClass('today'))
				{
					if (currentDate.getHours() > 15) {
						$('#ORDER_PROP_13 option[value="15"]').attr("disabled","");
					}
					
					if (currentDate.getHours() > 18) {
						$('#ORDER_PROP_13 option[value="18"]').attr("disabled","");
					}
				} else {
					$('#ORDER_PROP_13 option').attr("disabled", null);
				}
				
			});
		},
		
		insertDatePickupProperty: function(property, propsItemNode, disabled)
		{
			var prop, dateInputs, values, i,
				propContainer, inputText;
			...
			else
			{
				...
				{
					//console.log(property.getSettings());
					this.alterDatePickupProperty(property.getSettings(), inputText[i]);
				}
		...
		alterDatePickupProperty: function(settings, inputText)
		{
			var parentNode = BX.findParent(inputText, {tagName: 'DIV'});
			var propContainer = BX.create('DIV', {props: {className: 'date-delivery-select'}});
			...
			while(printedDays < 7) 
			{
				...
				if (today && !printedDays) {
					number = BX.create('p', {text: 'Сегодня'});
					day = BX.create('SPAN', {html: '&nbsp;'});
					month = BX.create('SPAN', {html: '&nbsp;'});
			...
			$(inputText).attr('type', 'hidden').attr('id', 'ORDER_PROP_10').val(firstDay);
			BX.remove(parentNode.querySelector('input[type=button]'));
			
			parentNode.appendChild(propContainer);
			
			
			$(document).on('click', '#data-property-id-row-10 .date-delivery-select div', function () {
				
				$('#ORDER_PROP_10').val($(this).data('prop-value'));
				$('.date-delivery-select div').removeClass('selected');
				$(this).addClass('selected');
				...
				if ($(this).hasClass('today'))
				{
					if (currentDate.getHours() > 12) {
						$('#ORDER_PROP_18 option[value="1"]').attr("disabled","");
					}
					
					if (currentDate.getHours() > 14) {
						$('#ORDER_PROP_18 option[value="2"]').attr("disabled","");
					}
					
					if (currentDate.getHours() > 16) {
						$('#ORDER_PROP_18 option[value="3"]').attr("disabled","");
					}
					
					if (currentDate.getHours() > 18) {
						$('#ORDER_PROP_18 option[value="4"]').attr("disabled","");
					}
				} else {
					$('#ORDER_PROP_18 option').attr("disabled", null);
				}
