		...
		if (strlen($arResult["ERROR_MESSAGE"]) <= 0 && $arResult["CurrentStep"] > 3)
		{
			...
			if (is_array($arResult["DELIVERY_ID"]))
			{
				...
				$arOrder = array(
					...
					"ITEMS" => $arResult["ITEMS"], // Added by pimax 22.08.2014
		...
		if (strlen($arResult["ERROR_MESSAGE"]) <= 0 && $arResult["CurrentStep"] > 5)
		{
			...
			if (strlen($arResult["ERROR_MESSAGE"]) <= 0)
			{
				...
				$arFields = Array(
					...
					"EMAIL" => ( (strlen($arResult["USER_EMAIL"]) > 0) ? $arResult["USER_EMAIL"] : $USER->GetEmail() ),
			...
			if (strlen($arResult["ERROR_MESSAGE"]) <= 0)
			{
				...
				LocalRedirect($arParams["PATH_TO_ORDER"]."?CurrentStep=7&Completed=Yes&ORDER_ID=".urlencode(urlencode($arOrder["ACCOUNT_NUMBER"])));
			}
