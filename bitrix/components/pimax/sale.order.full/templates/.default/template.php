	...
	if($arParams["SHOW_MENU"] == "Y")
	{
		...
		<tr>
			...
			<?if ($arResult["CurrentStep"] < 6):?>
				...
				<?
				$arMenuLine = array(
						GetMessage("STOF_PERSON_TYPE"),
						GetMessage("STOF_MAKING"),
						GetMessage("STOF_DELIVERY"),
						GetMessage("STOF_PAYMENT"),
						GetMessage("STOF_CONFIRM")
					);
				for ($i = 0; $i < count($arMenuLine); $i++)
				{
					...
					if (($arResult["SKIP_FIRST_STEP"] == "Y" && $i > 1) || ( $arResult["SKIP_FIRST_STEP"] != "Y" && $i > 0))
						echo " &gt; ";
