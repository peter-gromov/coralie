	...
	if ($arParams['arUserField']['SETTINGS']['LEAD'] == 'Y' && isset($arValue['LEAD']) && !empty($arValue['LEAD']))
	{
		...
		while ($arRes = $dbRes->Fetch())
		{
			$arResult['VALUE']['LEAD'][$arRes['ID']] = Array(
				'title' => $arRes['TITLE'],
				'url' => CComponentEngine::MakePathFromTemplate("/mobile/crm/lead/view.php?lead_id=#lead_id#", array('lead_id' => $arRes['ID']))
			);
		}
	}
	if ($arParams['arUserField']['SETTINGS']['CONTACT'] == 'Y' && isset($arValue['CONTACT']) && !empty($arValue['CONTACT']))
	{
		...
		while ($arRes = $dbRes->Fetch())
		{
			...
			$arResult['VALUE']['CONTACT'][$arRes['ID']] = Array(
				'title' => $title,
				'url' => CComponentEngine::MakePathFromTemplate("/mobile/crm/contact/view.php?contact_id=#contact_id#", array('contact_id' => $arRes['ID']))
			);
		}
	}
	if ($arParams['arUserField']['SETTINGS']['COMPANY'] == 'Y'
	&& isset($arValue['COMPANY']) && !empty($arValue['COMPANY']))
	{
		...
		while ($arRes = $dbRes->Fetch())
		{
			$arResult['VALUE']['COMPANY'][$arRes['ID']] = Array(
				'title' => $arRes['TITLE'],
				'url' => CComponentEngine::MakePathFromTemplate("/mobile/crm/company/view.php?company_id=#company_id#", array('company_id' => $arRes['ID']))
			);
		}
	}
	if ($arParams['arUserField']['SETTINGS']['DEAL'] == 'Y'
	&& isset($arValue['DEAL']) && !empty($arValue['DEAL']))
	{
		...
		while ($arRes = $dbRes->Fetch())
		{
			$arResult['VALUE']['DEAL'][$arRes['ID']] = Array(
				'title' => $arRes['TITLE'],
				'url' => CComponentEngine::MakePathFromTemplate("/mobile/crm/deal/view.php?deal_id=#deal_id#", array('deal_id' => $arRes['ID']))
			);
		}
	}
}
